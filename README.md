...
> **어디서든 Claude Code를 실행하고, 진행을 모니터링하고, 이어서 작업할 수 있는 도구**

- **멀티 디바이스** — 브라우저만 있으면 어떤 기기에서든 같은 작업 환경에 접속합니다. 데스크탑에서 배치한 세션이 폰에 그대로 보이고, 폰에서 한 작업이 데스크탑에 즉시 반영됩니다. Web Push 알림으로 자리를 비워도 Claude의 요청을 놓치지 않습니다
- **레이트 리밋 모니터링** — 5시간/7일 잔여량과 리셋 카운트다운을 사이드바에서 항상 표시합니다. 지금 밀어붙일지, 아껴 쓸지 바로 판단할 수 있습니다. 프로젝트별 비용 분석, 활동 패턴, AI 일일 리포트까지 제공합니다
- **인터랙티브 타임라인** — 터미널 출력을 스크롤하는 대신, Claude의 메시지·도구 호출·태스크 진행을 구조화된 뷰로 봅니다. 세션이 길어져도 원하는 시점을 바로 찾을 수 있고, 권한 승인이나 질문 답변도 타임라인에서 직접 처리합니다
