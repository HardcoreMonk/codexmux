...
const DEFAULT_TIMEOUT_MS = Number(process.env.CODEXMUX_RUNTIME_V2_TIMELINE_SESSION_CHANGED_TIMEOUT_MS || 30_000);
...
const main = async (): Promise<void> => {
  const homeDir = process.env.CODEXMUX_RUNTIME_V2_TIMELINE_SESSION_CHANGED_HOME
  ...
  const port = Number(process.env.CODEXMUX_RUNTIME_V2_TIMELINE_SESSION_CHANGED_PORT || await getFreePort());
