...
기본 포트는 **8022**입니다 (web + ssh 합성어). `server.ts`에서 `PORT` 환경변수로 오버라이드 가능합니다.
...
- Next.js 16부터 `middleware.ts`가 **`proxy.ts`**로 변경되었습니다. 이 프로젝트에서 `src/proxy.ts`가 인증 미들웨어 역할을 합니다
...
앱 설정(워크스페이스, 사용자 설정 등)은 **`~/.purplemux/workspaces.json`** 파일에 저장합니다:

```
✅ ~/.purplemux/workspaces.json (파일 시스템)
❌ 메모리(변수, 전역 상태만으로 관리)
...
| [docs/STATUS.md](./docs/STATUS.md)                     | Claude CLI 작업 상태 감지 로직 및 데이터 흐름      |
| [docs/TMUX.md](./docs/TMUX.md)                         | tmux 터미널 관리, 프로세스 감지, WebSocket 구조     |

**상태 관련 코드(`TClaudeStatus`, `TCliState`, `selectSessionView`, `setClaudeStatus` 등)를 수정하면 `docs/STATUS.md`도 함께 업데이트합니다.**
