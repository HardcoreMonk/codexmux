...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'GET') {
    const agents = manager.listAgents().map(({ id, name, role, status }) => ({
      id,
      name,
      role,
      status,
  ...
  if (req.method === 'POST') {
    const { name, role } = req.body as Partial<ICreateAgentRequest>;
    if (!name) {
      return res.status(400).json({ error: 'name은 필수입니다' });
    }

    try {
      const info = await manager.createAgent(name, role ?? '');
