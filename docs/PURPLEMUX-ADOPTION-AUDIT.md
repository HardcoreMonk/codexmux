...
> Codexmux 기준: `main@cafc8de9` (`v0.4.16`)
...
상태: **Codexmux 조치 완료, ADR-027 Implemented.** Legacy Pages upload route를 제거하고 exact
두 route를 outer custom server의 authenticated streaming ingress로 옮겼습니다. Fresh Windows
packaged filesystem 증거 전에는 ADR-027을 Verified로 올리지 않습니다.
...
Codexmux P0 구현은 production audit 0건, 1,426 unit/integration tests, build, pre-auth
production security smoke, 50MiB external-memory gate, dev/prod upload 각 12 checks, browser
reconnect, Electron build/runtime까지 Linux에서 통과했습니다. Windows packaged upload gate는
구현되어 package gate에 포함됐지만 이 감사 시점의 fresh Windows 실행 증거는 없습니다.

## 권장 실행 순서

1. **Security hotfix (Codexmux 구현 완료, Windows upload 증거 pending)**
...
bootstrap 경계로 제거했으며, 다음 우선순위는 fresh Windows upload 증거를 닫은 뒤 rate
limit과 rich timeline을 Runtime v2와 approval contract 안에 수동 통합하는 것입니다.
