...
이미 init을 보낸 같은 JSONL path에 대해 session watcher가 같은 session/path를 다시 보고하는
경우는 `timeline:session-changed`를 중복 송신하지 않아야 한다. 이 회귀는
`tests/unit/lib/runtime/timeline-ws.test.ts`에서 확인하며, 중복 이벤트가 init 뒤에 도착하면
client가 entries를 지우고 다음 init을 기다리는 CODEX panel skeleton 상태로 남을 수 있다.
...
Live UI 회귀 smoke에서는 같은 tab에서 `TERMINAL -> CODEX` 또는 `DIFF -> CODEX`로 전환한
뒤 timeline log 영역이 렌더링되고 상위 skeleton이 남지 않는지 확인한다. 이 경로는
client-local `agentSessionId`/`agentJsonlPath` 보존과 runtime v2 duplicate
`session-changed` suppression을 함께 검증한다.
