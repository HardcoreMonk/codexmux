...
일정 시간 이상 hidden 상태였거나 bfcache restore가 감지되면 terminal/status/timeline/sync WebSocket을 readyState와 관계없이 새로 연결한다. sync hook은 foreground 복귀 시 workspace/layout도 다시 가져온다.
