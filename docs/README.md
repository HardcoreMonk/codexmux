...
이 디렉터리는 codexmux의 내부 설계, 운영, 플랫폼 전환 기준 문서를 모읍니다. 현재 canonical 문서 언어는 한국어입니다.

사용자용 다국어 랜딩 문서는 `landing-src/docs/`에 보존합니다. 과거 실행 로그와 release handoff는 `docs/operations/`에 보존하며, 한국어 재작성 대상에서 제외합니다.
...
| `WINDOWS-ONLY-GAP-AUDIT.md` | Windows-only 제품 전환 gap, domain language, 전환 순서 |
...
| `PERFORMANCE.md` | perf snapshot, cache, polling, render 최적화 기준 |
...
| 문서 | 기준 |
| --- | --- |
| `ANDROID.md` | Android Capacitor shell 기록. Windows-only 전환 후 primary surface가 아님 |
...
| 문서 | 기준 |
| --- | --- |
| `agents/domain.md` | domain architecture pass와 ADR 소비 규칙 |
| `agents/issue-tracker.md` | issue tracker 조작 규칙 |
| `agents/triage-labels.md` | triage label/status mapping |

## 갱신 규칙

- Windows-only 제품 타깃, terminal runtime, process inspector, host/installer/update 정책을 바꾸면 `WINDOWS-ONLY-GAP-AUDIT.md`, `ADR.md`, 관련 `superpowers/specs/`와 `superpowers/plans/`를 함께 갱신합니다.
