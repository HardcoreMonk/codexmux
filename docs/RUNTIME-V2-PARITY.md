...
## 2026-05-07 CODEX Panel Timeline Hotfix Evidence

- `9433f1b` preserves `agentSessionId` and `agentJsonlPath` when switching between
  TERMINAL/CODEX/DIFF panel surfaces, and restores `sessionView=timeline` when stored Codex
  metadata exists.
- `923e9d6` suppresses duplicate runtime v2 `timeline:session-changed` messages when the session
  watcher reports the same JSONL path after `timeline:init`.
- Focused regression coverage:
  `corepack pnpm vitest run tests/unit/lib/runtime/timeline-ws.test.ts tests/unit/hooks/use-layout-panel-type.test.ts tests/unit/hooks/use-tab-store.test.ts tests/unit/lib/session-list-rendering.test.ts`.
- Runtime smoke coverage:
  `corepack pnpm smoke:runtime-v2:timeline-websocket-default` and
  `corepack pnpm smoke:runtime-v2:timeline-session-changed`.
- Live deploy evidence: `corepack pnpm deploy:local`, `/api/health` `version=0.4.7`,
  `commit=923e9d6`, systemd `ActiveState=active`, `SubState=running`, `NRestarts=0`.
