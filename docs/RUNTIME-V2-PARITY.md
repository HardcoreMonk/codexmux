...
| Surface | Owner | v1 behavior | v2 behavior | Gap | Migration | Test | Rollback |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Attach/detach/stdout | `src/lib/terminal-server.ts`, `src/lib/runtime/terminal-ws.ts`, `src/lib/terminal-websocket-url.ts` | `/api/terminal` WebSocket이 `pt-` tmux session에 attach하고 stdout을 fanout한다. | `/api/v2/terminal` WebSocket이 `rtv2-` session에 attach하고 Terminal Worker stdout event를 fanout한다. Desktop/mobile terminal surface는 `runtimeVersion: 2` tab에서 `/api/v2/terminal`을 선택한다. | web stdin, heartbeat, backpressure close, Electron/Android auth smoke가 남아 있다. | `CODEXMUX_RUNTIME_TERMINAL_V2_MODE=new-tabs`에서 plain terminal 신규 tab만 v2. | `CODEXMUX_RUNTIME_V2_SMOKE_URL=... node scripts/smoke-runtime-v2.mjs` + terminal URL helper tests. | mode off blocks new v2 tab creation; existing legacy tabs remain on `/api/terminal`. |
