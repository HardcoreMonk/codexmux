...
| Load older entries | `src/lib/timeline-server.ts`, `src/lib/session-parser.ts`, `src/lib/runtime/timeline/worker-service.ts` | timeline WebSocket/API reads JSONL entries with stable id/dedupe/merge. | v2 read-only older-entry command exists and `CODEXMUX_RUNTIME_TIMELINE_V2_MODE=default` routes legacy `/api/timeline/entries` through Supervisor/Timeline Worker. | live WebSocket default ownership missing. | keep client URL stable while read owner moves by env mode. | fixture tests for paired assistant dedupe, load-more, and default-read route unit tests. | `CODEXMUX_RUNTIME_TIMELINE_V2_MODE=off` returns older-entry reads to provider direct read. |
| Init/append subscribe/unsubscribe | `src/lib/timeline-server.ts`, `src/lib/timeline-server-state.ts`, `src/lib/runtime/timeline-live-shadow.ts`, `src/lib/runtime/timeline/worker-service.ts` | WebSocket sends init, append, error and manages watchers/subscribers. | `CODEXMUX_RUNTIME_TIMELINE_V2_MODE=shadow` starts worker live subscriptions behind legacy `/api/timeline`; worker init is returned from `timeline.live-subscribe`, append/error are runtime IPC events, and sanitized parity counters are recorded without changing client delivery. Android WebView page-context smoke proves foreground reconnect gets fresh timeline init after background JSONL append. | session-changed/resume/default WebSocket ownership remain. | keep legacy client-facing path in shadow, then promote after default-owned WebSocket rollback evidence. | runtime IPC/supervisor/worker live unit tests + `corepack pnpm smoke:runtime-v2:timeline-live-shadow` + `corepack pnpm smoke:android:timeline-foreground`. | `CODEXMUX_RUNTIME_TIMELINE_V2_MODE=off` disables worker live shadow; clients stay on legacy `/api/timeline`. |
| Resume/session-changed | `src/lib/timeline-server.ts`, `src/lib/auto-resume.ts`, `src/lib/runtime/timeline/worker-service.ts`, `src/lib/runtime/supervisor.ts` | resume flow blocks unsafe active processes and broadcasts session-changed. | v2 has an internal typed session watcher contract: Timeline Worker accepts `timeline.session-watch-subscribe`/`timeline.session-watch-unsubscribe`, emits subscriber-scoped `timeline.session-changed`, and Supervisor fans out matching changes. Resume execution and client-facing `/api/timeline` WebSocket delivery do not move yet. | default WebSocket ownership still needed. | keep resume/session-changed delivery in legacy until default WebSocket bridge is proven. | `corepack pnpm smoke:runtime-v2:timeline-resume-safety` proves `timelineV2Mode=default` still blocks unsafe active processes before resume execution. `corepack pnpm smoke:runtime-v2:timeline-session-changed` proves legacy event ordering: `session-changed` before new JSONL init. `corepack pnpm test tests/unit/lib/runtime/ipc.test.ts tests/unit/lib/runtime/timeline-worker-service.test.ts tests/unit/lib/runtime/supervisor.test.ts` proves the v2 watcher IPC contract. | legacy resume/session watcher flow. |
