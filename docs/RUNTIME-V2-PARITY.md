...
- `src/lib/runtime/storage-mirror.ts`는 `write|default` 모드에서 legacy JSON workspace/layout write 직후 `importLegacyStorageSnapshot`을 실행해 SQLite projection을 갱신하고, JSON snapshot에서 사라진 workspace/tab/pane/group row를 prune한다.
