...
| Surface | Owner | v1 behavior | v2 behavior | Gap | Migration | Test | Rollback |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Attach/detach/stdout | `src/lib/terminal-server.ts`, `src/lib/runtime/terminal-ws.ts`, `src/lib/terminal-websocket-url.ts`, `src/lib/terminal-runtime-preflight.ts` | `/api/terminal` WebSocket이 `pt-` tmux session에 attach하고 stdout을 fanout한다. | `/api/v2/terminal` WebSocket이 `rtv2-` session에 attach하고 Terminal Worker stdout event를 fanout한다. Desktop/mobile terminal surface는 `runtimeVersion: 2` tab에서 `/api/v2/terminal`을 선택한다. Server smoke covers stdin, web stdin, heartbeat, resize, fresh reattach, fanout, backpressure close, and cleanup rejection. Phase 2 gate smoke covers app layout new-tab routing, browser reload, server restart, and mode-off fallback. | Electron/Android auth and foreground reconnect smoke가 남아 있다. | `CODEXMUX_RUNTIME_TERMINAL_V2_MODE=new-tabs`에서 plain terminal 신규 tab만 v2. | `CODEXMUX_RUNTIME_V2_SMOKE_URL=... node scripts/smoke-runtime-v2.mjs`, `node scripts/smoke-runtime-v2-phase2-gate.mjs`, and terminal URL/preflight helper tests. | mode off blocks new v2 tab creation; existing v2 tabs remain visible and show `runtime-v2-disabled` before opening `/api/v2/terminal`. Existing legacy tabs remain on `/api/terminal`. |
