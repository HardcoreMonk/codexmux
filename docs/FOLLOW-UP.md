...
- `corepack pnpm ops:backlog:completion-gate`는 batch-run artifact와 선택적 manifest를 읽어 전체 backlog close 가능 여부를 판정한다. `skipped` row는 완료로 세지 않으며, 모든 row가 `passed`, `evidence-attached`, `spec-linked`, `approved-deferred` 중 하나로 닫힐 때만 `completionPercent: 100`과 `closable: true`를 반환한다.
- `corepack pnpm ops:backlog:completion-manifest`와 `corepack pnpm ops:backlog:complete`는 full backlog closeout을 자동화한다. `CODEXMUX_BACKLOG_COMPLETION_ALLOW_DEFER=1`이 없으면 manual/external row를 닫지 않고, 이 flag가 있을 때만 owner/reason/revisit trigger가 포함된 `approved-deferred` manifest로 completion gate 100% closeout을 허용한다.
