...
- `status-manager.ts`는 client payload helper, 순수 정책 helper, hook event intent helper, JSONL idle scan/watch/reconciliation helper, poll service, scan/bootstrap 및 poll workspace traversal helper, poll tab reconciliation/helper, poll tab entry update helper, tab entry construction helper, pane recovery service, resolve-unknown decision helper, stop recheck scheduler, session history entry/persistence helper, Web Push payload/delivery helper를 분리했다. 다음 단계에서는 poll stuck-busy/recovery side effect body를 더 얇게 유지한다.
