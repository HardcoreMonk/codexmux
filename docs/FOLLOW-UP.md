...
| runtime v2 storage shadow/dry-run/backup | 부분 통과 | `corepack pnpm smoke:runtime-v2:storage-shadow`, legacy JSON에 mirror된 `runtimeVersion: 2` tab과 SQLite runtime layout projection read-only compare 통과. `corepack pnpm smoke:runtime-v2:storage-dry-run`, live `corepack pnpm runtime-v2:storage-dry-run`, `corepack pnpm smoke:runtime-v2:storage-backup`, live `corepack pnpm runtime-v2:storage-backup` 통과. live dry-run은 `cutoverReady=false`, workspace group/legacy tab/non-terminal tab/status metadata blocker를 민감 값 없이 출력. live backup은 29개 JSON/SQLite 파일을 `runtime-v2-storage-20260504T052000Z`에 복사. full import/write ownership은 남음 |
...
- P2 남음: packaged Electron foreground/reconnect를 Mac 화면 세션에서 evidence로 보존, runtime v2 shadow 24시간 worker restart-loop 부재 관찰, runtime v2 storage full JSON-to-SQLite import/write ownership과 dry-run blocker 해소, timeline/status parity surface별 cutover evidence, release workflow/CI에서 선택 실행할 Android/Electron/browser reconnect smoke artifact 보존.
