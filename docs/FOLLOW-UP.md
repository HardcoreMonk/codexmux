...
- `status-manager.ts`는 client payload helper, 순수 정책 helper, hook event intent helper, JSONL idle scan/watch helper, poll service, poll tab reconciliation helper, pane recovery service, session history entry/persistence helper, Web Push payload/delivery helper를 분리했다. 다음 단계에서는 poll loop의 workspace/layout traversal orchestration만 남기고 tab entry construction helper를 추가 분리한다.
