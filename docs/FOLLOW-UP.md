...
- runtime v2 Phase 6 default gate: `smoke:runtime-v2:phase6-default-gate`가 `/api/v2/runtime/health`와 `/api/debug/perf`를 read-only로 조회해 terminal `new-tabs`, storage/timeline/status `default`, worker health ok, failure/restart/timeout counter 0을 확인한다. 이 gate는 code fallback default를 바꾸지 않는다.
...
- P3 남음: 필요 시 rollback drill, 측정 기반 perf tuning. Lifecycle control UI는 read-only 1차 범위를 완료했고, 실행형 control은 별도 spec으로 분리한다. Timeline Phase 4 WebSocket default ownership과 Status Phase 5 live bridge는 temp smoke 기준 완료됐고, Phase 6은 `smoke:runtime-v2:phase6-default-gate`로 live/default 상태를 확인한 뒤 code fallback default 변경 여부를 별도 승인으로 결정한다.
