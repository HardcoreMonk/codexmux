...
- `status-manager.ts`는 client payload helper, 순수 정책 helper, hook event intent helper, JSONL idle scan/watch helper, poll service, poll tab reconciliation/helper, tab entry construction helper, pane recovery service, session history entry/persistence helper, Web Push payload/delivery helper를 분리했다. 다음 단계에서는 poll loop의 workspace/layout traversal orchestration을 더 얇게 유지하면서 resolve-unknown/recheck-stop 보정 흐름을 추가 분리한다.
