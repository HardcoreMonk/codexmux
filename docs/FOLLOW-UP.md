...
| runtime v2 status shadow | 부분 통과 | `corepack pnpm smoke:runtime-v2:status-shadow`, Status Worker IPC reducer/policy/side-effect intent output과 legacy pure helper output compare 통과. polling/ack/Web Push/session history ownership은 남음 |
...
- P3 남음: 필요 시 rollback drill, 측정 기반 perf tuning. Lifecycle control UI는 read-only 1차 범위를 완료했고, 실행형 control은 별도 spec으로 분리한다. Timeline Phase 4 WebSocket default ownership은 temp smoke 기준 완료됐고, Status Phase 5는 `docs/RUNTIME-V2-CUTOVER.md`의 별도 gate로 진행한다.
