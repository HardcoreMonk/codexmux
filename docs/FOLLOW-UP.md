...
| runtime v2 storage shadow/dry-run | 부분 통과 | `corepack pnpm smoke:runtime-v2:storage-shadow`, legacy JSON에 mirror된 `runtimeVersion: 2` tab과 SQLite runtime layout projection read-only compare 통과. `corepack pnpm smoke:runtime-v2:storage-dry-run` 및 live `corepack pnpm runtime-v2:storage-dry-run` 통과. live dry-run은 `cutoverReady=false`, workspace group/legacy tab/non-terminal tab/status metadata blocker를 민감 값 없이 출력. full import/write ownership은 남음 |
