...
Windows terminal bridge의 source 등록, command queue, recent output snapshot은 `~/.codexmux/`에 저장하지 않고 server process memory의 `globalThis.__ptRemoteTerminalStore`에만 둔다. bridge 인증은 같은 `cli-token`을 `x-cmux-token`으로 사용하며, Windows 쪽 token 파일은 보통 `%USERPROFILE%\.codexmux\cli-token`에 둔다. 서버 재시작 후에는 Windows bridge가 다시 register/poll하면서 `/api/remote/terminal/sources` 목록이 복구된다.
...
`remote/codex/**/*.jsonl`을 읽기 전용으로 사용하며, message count cache는 worker
process memory에만 둔다.
