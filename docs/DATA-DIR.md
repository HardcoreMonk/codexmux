...
| `session-index.json` | Codex JSONL session list metadata cache |
...
| `approval-audit.jsonl` | sanitized approval action event |
...
비밀번호만 초기화하려면 `authPassword`, `authSecret`만 제거합니다. `config.json` 전체를 삭제하면 locale, theme, network, Codex option도 함께 초기화됩니다.
...
| `session-index.json` | session list cache 재생성 |
