...
이 저장소에서 새 기능, behavior change, workflow contract change, multi-file change를 시작할 때는 domain architecture pass를 거칩니다.
...
2. `CONTEXT-MAP.md`
3. `docs/adr/`
4. context-specific ADR directory
5. `docs/ADR.md`
6. 관련 기존 코드와 문서
...
- adapter/infrastructure boundary 영향
- ADR 후보

## 용어

확인된 domain language와 충돌하는 이름을 코드에 추가하지 않습니다. Windows-only 전환에서는 다음 용어를 우선합니다.
...
| Runtime adapter | OS별 terminal/process/service 구현 경계 |
...
- Windows bridge
- tmux backend를 canonical runtime term으로 쓰는 표현
...
`improve-codebase-architecture`는 accepted plan에 포함된 architecture 후보에만 적용합니다.
