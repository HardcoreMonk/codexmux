...
- Electron은 `build:electron`, live attach smoke, Mac M1 `pack:electron:dev` 산출물 검증까지 통과했다. SSH 세션의 macOS GUI launch domain 제한 때문에 Finder 더블클릭 실행과 Gatekeeper UX는 Mac 화면 세션에서 별도로 확인한다.
