...
현재 repo version은 `0.3.1`이며 Android 설치 상태는 `versionName=0.3.1`, `versionCode=301`이어야 합니다.
...
현재 `0.3.1` debug install은 `dumpsys package`에서 `versionName=0.3.1`, `versionCode=301`로 보여야 합니다.
