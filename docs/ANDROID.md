...
현재 repo version은 `0.3.2`이며 다음 Android 빌드/설치 후 상태는 `versionName=0.3.2`, `versionCode=302`가 됩니다.
...
현재 `0.3.2` debug install은 `dumpsys package`에서 `versionName=0.3.2`, `versionCode=302`로 보여야 합니다.
