...
- 현재 source의 Windows package gate에는 packaged upload integrity가 포함됐으며, fresh
  Windows 재실행은 [Issue #16](https://github.com/HardcoreMonk/codexmux/issues/16)에서 추적합니다.
...
Fresh Windows가 아닌 platform의 skipped package 결과는 새 release 증거가 아닙니다. ADR-027은
Issue #16의 hard-link/delete/cleanup/kill-switch package evidence가 확보될 때까지
`Implemented`로 유지합니다.
