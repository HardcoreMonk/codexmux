...
Storage schema v2 import는 legacy `layout.json`의 `cliState`, `agentSessionId`,
...
session 선택은 tmux process 제어가 아니라 저장된 JSONL 구독이다. Windows `pwsh` 제어는
별도 `/windows-terminal` page와 `/api/remote/terminal` bridge 경로에서 처리하므로
`TCliState`/`agentProcess` 판정에 섞지 않는다. `codex` panel의
