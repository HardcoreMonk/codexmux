...
`src/lib/status/poll-tab-reconciliation.ts`가 담당한다. initial `ITabStatusEntry` construction은
`src/lib/status/tab-entry.ts`가 담당한다. Codex permission/input prompt pane capture
