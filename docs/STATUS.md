...
`CODEXMUX_RUNTIME_STATUS_V2_MODE=default`에서만
`src/lib/status/session-history-persistence.ts` adapter가 사용한다.
...
Web Push title/body/silent/approval metadata payload construction은
`src/lib/status/web-push-payload.ts`의 pure helper가 담당한다. `StatusManager`는 workspace/config를
조회하고, foreground/runtime-v2/legacy send orchestration은
`src/lib/status/web-push-delivery.ts` helper에 위임한다.
...
history 저장, notification trigger 같은 부수효과를 처리한다. session history entry 생성,
session history runtime-v2/legacy persistence fallback, Web Push payload 생성,
runtime-v2/legacy delivery fallback은 status 전용 helper로 분리되어 단위 테스트한다.
poll timer lifecycle, adaptive interval, poll snapshot/duration recording은
`src/lib/status/poll-service.ts`가 담당한다. poll tab initial CLI state, synthetic needs-input
baseline event, process retry countdown, terminal port comparison, broadcast-needed aggregation은
...
결정된 상태 변경을 적용하고 layout/broadcast side effect만 수행한다. JSONL file watch
lifecycle, debounce, replacement, close/error cleanup은 `src/lib/status/jsonl-watch-service.ts`가
담당하고, JSONL 내용 변경 후 상태 reconciliation은 `StatusManager.onJsonlFileChange()`에 남아 있다.
raw hook event의 compact/ignore/transition classification, event sequence 생성, Codex stop
defer와 JSONL resolve/refresh intent는 `src/lib/status/hook-event-service.ts`가 담당한다.
`unknown` 상태 보정 decision은 `src/lib/status/resolve-unknown-service.ts`가 담당하고,
Codex stop 후 지연 JSONL 재검증과 stop snippet refresh scheduling은
`src/lib/status/stop-recheck-service.ts`가 담당한다. JSONL file-change watch 유지,
synthetic interrupt freshness, delayed pane recovery scheduling predicate는
`src/lib/status/jsonl-reconciliation-service.ts`가 담당한다.
`StatusManager.updateTabFromHook()`는 intent에 따라 state mutation, layout persist, broadcast,
JSONL resolve/recheck side effect를 수행한다.
client sync/update/remove payload projection은 `src/lib/status/client-payload.ts`가 담당하고,
`StatusManager`는 WebSocket transport와 bridge trace forwarding만 수행한다.
...
| 파일 | 역할 |
|---|---|
| `src/lib/status-manager.ts` | 서버 상태 orchestration, tab map, JSONL reconciliation, broadcast |
| `src/lib/status/client-payload.ts` | status sync/update/remove client payload projection helper |
| `src/lib/status/hook-event-service.ts` | hook event compact/ignore/transition intent helper |
...
| `src/lib/status/stop-recheck-service.ts` | delayed Codex stop recheck and stop snippet refresh scheduler |
| `src/lib/status/tab-entry.ts` | initial internal tab status entry construction helper |
| `src/lib/status/session-history-entry.ts` | session history prompt/result/duration/touched file entry builder |
| `src/lib/status/session-history-persistence.ts` | runtime v2 session history write/update fallback adapter |
| `src/lib/status/web-push-payload.ts` | Web Push title/body/silent/approval metadata payload helper |
| `src/lib/status/web-push-delivery.ts` | runtime v2 Web Push send fallback과 legacy delivery orchestration helper |
