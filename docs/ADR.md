...
- 검증 근거: Linux에서 full unit suite, lint, typecheck, production/landing build, dev/prod pre-auth bootstrap과 upload integrity smoke, Chromium same-host cookie coexistence/reconnect smoke가 통과했습니다. 상세 결과는 `docs/operations/2026-07-12-purplemux-cookie-isolation-handoff.md`에 기록합니다. Fresh Windows packaged app의 `v0.4.21 -> next` update, 1회 재로그인, runtime v2 WebSocket와 upload 재연결을 확인한 뒤 `Verified`로 전이합니다.
