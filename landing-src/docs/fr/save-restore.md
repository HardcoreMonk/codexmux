...
`workspaces.json`은 레이아웃 변경 때마다 트랜잭션 단위로 업데이트되므로 파일은 항상 현재 상태를 반영합니다. 디스크 파일 구조는 [데이터 디렉토리](/codexmux/fr/docs/data-directory/)를 참고하세요.
...
이진 프로토콜, 백프레셔, JSONL 워처 같은 더 깊은 이야기는 랜딩의 [동작 방식](/codexmux/fr/#how) 섹션을 참고하세요.

## 다음으로

- **[워크스페이스와 그룹](/codexmux/fr/docs/workspaces-groups/)** — 워크스페이스 단위로 저장되는 것
- **[탭 & 창](/codexmux/fr/docs/tabs-panes/)** — 탭 단위로 저장되는 것
- **[브라우저 지원](/codexmux/fr/docs/browser-support/)** — 모바일 백그라운드 탭과 재연결의 알려진 이슈
