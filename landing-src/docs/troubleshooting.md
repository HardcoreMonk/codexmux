...
### CODEX 메뉴가 계속 연결 중처럼 보여요

TERMINAL 또는 DIFF에서 CODEX로 돌아오면 같은 Codex session id와 JSONL path를 다시 사용해
타임라인이 보여야 합니다. 배포 직후 이미 열린 브라우저 탭이나 Android WebView는 예전
JavaScript chunk를 계속 들고 있을 수 있으니 먼저 새로고침하거나 앱을 재시작하세요.

새로고침 후에도 계속 skeleton만 보이면 `/api/health`의 `commit`이 최신 배포인지 확인하고,
운영 smoke로 `corepack pnpm smoke:runtime-v2:timeline-websocket-default`와
`corepack pnpm smoke:runtime-v2:timeline-session-changed`를 실행하세요. 이 두 smoke는
timeline init/append와 늦게 생성되는 JSONL의 session-changed 순서를 확인합니다.
