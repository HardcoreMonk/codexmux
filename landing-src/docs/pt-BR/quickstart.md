...
서버 실행은 macOS와 Linux를 기준으로 지원합니다. Windows 11은 서버 실행 대상이 아니지만, `pwsh`에서 생성된 Codex CLI JSONL을 실행 중인 codexmux 서버로 동기화하는 companion client를 제공합니다. 자세한 설정은 저장소의 `docs/WINDOWS.md`를 참고하세요.
{% endcall %}
