...
전체 호환성 매트릭스는 [브라우저 지원](/codexmux/es/docs/browser-support/) 참고.
...
**설정 → 브라우저 체크**를 실행해 API별 리포트를 보세요. [브라우저 지원](/codexmux/es/docs/browser-support/)의 최소 버전 미만은 기능을 그레이스풀하게 잃지만 공식 지원은 아닙니다.
...
또는 앱의 **설정 → 네트워크 접근** (이 값은 `~/.codexmux/config.json`에 기록). env로 `HOST`를 지정한 경우 이 필드는 잠깁니다. 키워드와 CIDR 문법은 [포트 & 환경변수](/codexmux/es/docs/ports-env-vars/) 참고.
...
전부 로컬 `~/.codexmux/` 안. 외부로 나가는 데이터는 없습니다. 로그인 비밀번호는 `config.json` 안의 scrypt 해시. 전체 구조는 [데이터 디렉토리](/codexmux/es/docs/data-directory/) 참고.
...
- **[브라우저 지원](/codexmux/es/docs/browser-support/)** — 자세한 호환성 매트릭스와 알려진 quirk
- **[데이터 디렉토리](/codexmux/es/docs/data-directory/)** — 각 파일의 역할과 삭제 안전성
- **[아키텍처](/codexmux/es/docs/architecture/)** — 더 깊이 파야 할 때 컴포넌트가 어떻게 맞물리는지
