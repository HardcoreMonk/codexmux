...
서버 실행은 macOS와 Linux를 기준으로 지원합니다. Windows 11은 서버 실행 대상이 아니지만, `pwsh`에서 생성된 Codex CLI JSONL을 실행 중인 codexmux 서버로 동기화하고 별도 `pwsh` terminal bridge를 실행하는 companion client 경로를 제공합니다. 자세한 설정은 저장소의 `docs/WINDOWS.md`를 참고하세요.
{% endcall %}
...
자세한 설정은 [Tailscale 접속](/codexmux/zh-CN/docs/tailscale/), iOS/Android 구체 방법은 [PWA 설정](/codexmux/zh-CN/docs/pwa-setup/)을 참고하세요.

## 다음으로

- **[설치](/codexmux/zh-CN/docs/installation/)** — 플랫폼별 세부 사항, macOS 네이티브 앱, 자동 시작
- **[브라우저 지원](/codexmux/zh-CN/docs/browser-support/)** — 데스크탑/모바일 호환성 매트릭스
- **[첫 세션](/codexmux/zh-CN/docs/first-session/)** — 대시보드 투어
- **[키보드 단축키](/codexmux/zh-CN/docs/keyboard-shortcuts/)** — 전체 바인딩 한눈에
