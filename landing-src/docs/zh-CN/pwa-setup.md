...
시작 화면은 `public/splash/*.png` 정적 이미지에서 내려오며 `scripts/generate-splash.js`가 `codexmux` branding으로 생성합니다. 이미 홈 화면에 추가한 앱이 이전 splash를 계속 보이면 iOS cache 때문에 홈 화면 앱을 삭제한 뒤 다시 추가하세요.

## 설치 전 확인

페이지가 **HTTPS**로 접근 가능해야 PWA가 동작합니다. `localhost`는 Chrome에서는 예외적으로 허용되지만 iOS Safari는 평문 HTTP에서 설치를 허용하지 않습니다. 가장 깔끔한 경로는 Tailscale Serve입니다 — [Tailscale 접속](/codexmux/zh-CN/docs/tailscale/) 참고.

{% call callout('warning', 'iOS는 Safari 16.4 이상 필요') %}
이전 iOS에서도 PWA 설치는 됩니다만 Web Push는 동작하지 않습니다. 푸시가 중요하다면 iOS를 먼저 업데이트하세요. 브라우저별 자세한 호환성은 [브라우저 지원](/codexmux/zh-CN/docs/browser-support/)에 정리되어 있습니다.
{% endcall %}
...
별도 Capacitor Android 앱을 설치한 경우에는 저장 서버/최근 서버 런처를 먼저 거칩니다. 서버 접속 후 모바일 메뉴의 앱 정보에서 Android `versionName`/`versionCode`, 패키지, 기기/Android 버전, 서버 버전을 확인하고 앱 재시작으로 WebView/Activity를 다시 열 수 있습니다.
...
Capacitor Android 앱도 React UI만 바뀐 경우에는 Linux user service 기준 `corepack pnpm deploy:local`만 필요합니다. `android-web/` 런처나 native bridge, Android version metadata가 바뀐 경우에는 APK를 다시 빌드하고 설치해야 합니다.

제거하려면 아이콘을 길게 눌러 OS 기본 삭제 동작을 사용하세요.

## 다음으로

- **[웹 푸시 알림](/codexmux/zh-CN/docs/web-push/)** — PWA 설치 이후 백그라운드 알림 켜기
- **[Tailscale 접속](/codexmux/zh-CN/docs/tailscale/)** — iOS가 요구하는 HTTPS URL 확보하기
- **[브라우저 지원](/codexmux/zh-CN/docs/browser-support/)** — 전체 호환성 매트릭스
