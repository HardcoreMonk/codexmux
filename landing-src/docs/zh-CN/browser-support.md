...
Capacitor Android 앱은 같은 codexmux 서버를 WebView로 열지만, 저장 서버 런처와 앱 정보/재시작 native bridge를 추가로 제공합니다. React UI만 바뀐 경우에는 서버 재빌드와 재시작으로 반영되고, native bridge나 런처 asset이 바뀐 경우에는 APK를 다시 설치해야 합니다.
...
- **모바일 브라우저/WebView + 백그라운드** — iOS Safari는 WebSocket을 끊고 Android WebView는 객체가 `OPEN`이어도 TCP가 죽을 수 있습니다. tmux는 실제 세션을 계속 유지하므로 foreground 복귀 시 terminal/status/timeline/sync 연결을 강제로 재확인합니다.
...
특이한 환경에서 문제가 발생하면 유저 에이전트와 자체 진단 결과를 첨부해 [이슈를 열어주세요](https://github.com/HardcoreMonk/codexmux/issues).
