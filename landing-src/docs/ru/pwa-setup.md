---
title: Настройка PWA
description: Добавьте purplemux на главный экран в iOS Safari и Android Chrome для полноэкранного, app-подобного опыта.
...
Страница должна быть доступна по **HTTPS**, чтобы PWA работал. С `localhost` это работает в Chrome (исключение для loopback), но iOS Safari не даст установить по обычному HTTP. Чистый путь — Tailscale Serve, см. [Доступ через Tailscale](/purplemux/ru/docs/tailscale/).

{% call callout('warning', 'iOS требует Safari 16.4 или новее') %}
Более ранние релизы iOS могут установить PWA, но Web Push доставляться не будет. Если push для вас важен, сначала обновите iOS. Подробности по браузерам — в [Поддержке браузеров](/purplemux/ru/docs/browser-support/).
{% endcall %}

## iOS Safari

1. Откройте URL purplemux в **Safari** (другие iOS-браузеры не показывают «На экран Домой» для PWA).
...
5. Запустите purplemux с новой иконки — он откроется на весь экран.
...
1. Откройте URL purplemux в **Chrome**.
...
- **[Web Push уведомления](/purplemux/ru/docs/web-push/)** — включите фоновые алерты, теперь когда PWA установлен.
- **[Доступ через Tailscale](/purplemux/ru/docs/tailscale/)** — получите HTTPS-URL, который требует iOS.
- **[Поддержка браузеров](/purplemux/ru/docs/browser-support/)** — полная матрица совместимости.
