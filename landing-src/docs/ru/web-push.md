...
3. purplemux зарегистрирует подписку Web Push под VAPID-ключи сервера.
...
На iPhone и iPad Web Push работает только после того, как вы добавили purplemux на главный экран и запустили его с этой иконки. Открывайте страницу Настройки из standalone-окна PWA — запрос разрешения на уведомления в обычной вкладке Safari ничего не сделает. Сначала настройте PWA: [Настройка PWA](/purplemux/ru/docs/pwa-setup/).
{% endcall %}
...
Уведомление приходит независимо от того, на переднем плане purplemux или нет. Если панель в данный момент видна на _каком-нибудь_ из ваших устройств, purplemux пропустит push, чтобы избежать двойного жужжания.
...
- **Переключатель серый** — Service Workers или Notifications API не поддерживаются. Запустите **Настройки → Проверка браузера** или см. [Поддержка браузеров](/purplemux/ru/docs/browser-support/).
...
- **Самоподписанный сертификат** — Web Push откажется регистрироваться. Используйте Tailscale Serve или reverse-proxy с настоящим сертификатом. См. [Доступ через Tailscale](/purplemux/ru/docs/tailscale/).

## Что дальше

- **[Настройка PWA](/purplemux/ru/docs/pwa-setup/)** — обязательна для push на iOS.
- **[Доступ через Tailscale](/purplemux/ru/docs/tailscale/)** — HTTPS для внешней доставки.
- **[Безопасность и аутентификация](/purplemux/ru/docs/security-auth/)** — что ещё лежит в `~/.purplemux/`.
