...
description: Как purplemux перехватывает диалоги Claude Code «можно это запустить?» и позволяет одобрять их с панели, с клавиатуры или с телефона.
eyebrow: Claude Code
...
Это и есть основная причина настроить [Tailscale + PWA](/purplemux/ru/docs/quickstart/#dostup-so-smartfona) — одобрения следуют за вами вне рабочего стола.
...
- **[Статус сессии](/purplemux/ru/docs/session-status/)** — что значит состояние **needs-input** и как оно определяется.
- **[Живой вид сессии](/purplemux/ru/docs/live-session-view/)** — где рендерится inline-запрос.
- **[Поддержка браузеров](/purplemux/ru/docs/browser-support/)** — требования Web Push (особенно iOS Safari 16.4+).
